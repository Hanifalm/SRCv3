...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    try:
        ...
        # Pengecekan ukuran file untuk upload:
...
async def clone_message(app, msg, target_chat_id, topic_id, edit_id, log_group):
    ...
    cloned = await app.send_message(target_chat_id, msg.text.markdown, reply_to_message_id=topic_id)
    await cloned.copy(log_group)
    await edit.delete()
...
async def callback_query_handler(event):
    ...
    if event.data == b'setchat':
        ...
        await event.edit("Choose your preferred upload method:\n\n__**Note:** **SpyLib ⚡**, built on Telethon, by Team SPY still in beta.__", buttons=buttons)
...
async def handle_large_file(file, sender, edit, caption):
    # Jika tidak menemukan konektor 4GB, beri pesan error dan hapus file
    if pro is None:
    ...
    try:
        ...
        freecheck = 0
        if freecheck == 1:
            reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("💎 Get Premium to Forward", url="https://t.me/kingofpatal")]])
            await app.copy_message(target_chat_id, from_chat, msg_id, protect_content=True, reply_markup=reply_markup)
        else:
...
async def split_and_upload_file(app, sender, target_chat_id, file_path, caption, topic_id):
    ...
    PART_SIZE = int(1.9 * 1024 * 1024 * 1024)  # Pastikan PART_SIZE adalah integer
    part_number = 0
